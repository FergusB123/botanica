...
router.get('/me', auth, async (req, res) => {
  try {
    const { rows } = await pool.query('SELECT id, email, name, created_at, push_subscription FROM users WHERE id = $1', [req.user.id]);
...
router.put('/profile', auth, async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query('SELECT id, email, name, created_at FROM users WHERE id = $1', [req.user.id]);
    res.json({ user: rows[0] });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
